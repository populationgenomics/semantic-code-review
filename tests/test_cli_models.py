...
async def test_subprocess_model_retries_until_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    bad = _FakeProc(json.dumps({"retry": True, "reason": "bad-1"}).encode("utf-8"))
    good = _FakeProc(
...
async def test_subprocess_model_raises_after_exhausting_retries(
    ...
    bad1 = _FakeProc(json.dumps({"retry": True, "reason": "bad-1"}).encode("utf-8"))
    bad2 = _FakeProc(json.dumps({"retry": True, "reason": "bad-2"}).encode("utf-8"))
    _install_claude_subproc(monkeypatch, [bad1, bad2])
...
async def test_subprocess_model_stdin_routing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    proc_fed = _FakeProc(json.dumps({"structured": {"intent": "ok"}}).encode("utf-8"))
    _install_claude_subproc(monkeypatch, [proc_fed])
    ...
    proc_dn = _FakeProc(json.dumps({"structured": {"intent": "ok"}}).encode("utf-8"))
    _install_claude_subproc(monkeypatch, [proc_dn])
