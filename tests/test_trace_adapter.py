...
def test_the_turn_budget_records_the_cap_and_what_was_spent(tmp_path: Path) -> None:
    """An agentic pass whose cost is invisible is the failure the cap
    exists to prevent — so both numbers land in the trace, not just the
    one the code already knew."""
    messages = [
        ModelRequest(parts=[UserPromptPart(content="q", timestamp=_ts())], timestamp=_ts()),
        ModelResponse(
            parts=[ToolCallPart(tool_name="read_file", args={"path": "a.py"}, tool_call_id="c1")],
        ...
        ModelRequest(
            parts=[ToolReturnPart(tool_name="read_file", content="x", tool_call_id="c1", timestamp=_ts())],
            timestamp=_ts(),
        ),
        ModelResponse(
...
def test_a_pass_that_declared_no_ceiling_records_no_budget(tmp_path: Path) -> None:
    trace_path = tmp_path / "nobudget.json"
    write_partial_trace([], trace_path=trace_path, model="m", system="s", tool_names=[], submit_tool="submit")
    assert "turn_budget" not in json.loads(trace_path.read_text())
