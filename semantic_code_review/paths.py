...
def runs_cache_root() -> Path:
    """The directory every repo's runs root sits under:
    `~/.cache/scr/runs/` (XDG-aware). `scr runs ps` walks it to see
    every repo's servers.
    """
    return Path(os.environ.get("XDG_CACHE_HOME") or (Path.home() / ".cache")) / "scr" / "runs"


def default_runs_root() -> Path:
    """Resolve the per-repo run-artefacts root for the current cwd.

    `<runs_cache_root()>/<fingerprint>/`, where the fingerprint is a
    sha256 of the resolved `git rev-parse --git-common-dir`. Worktrees
    of the same repo share a fingerprint; different repos get different
    ones. Falls back to a hash of cwd when not in a git repo (e.g. `scr
    pr` outside a checkout).
