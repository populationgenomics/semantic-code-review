...
_ANNOTATION_FIELDS = (
    ...
    "segments if the hunk is single-intent. Every hunk block states its post-image range "
    "as `+first..+last`, inclusive of BOTH ends — do not recompute it from the `@@` "
    "header. A segment's last line is `new_start + new_count - 1`, and it must not exceed "
    "`+last`. Segments must not overlap: the next one starts one line after the previous "
    "one's last line, never on it.\n"
