...
_ANNOTATION_FIELDS = (
    ...
    "- `spans`: labelled ranges of the hunk. Each span is {start, end, intent, smells, "
    "context, refs}, where `start` and `end` are BOUNDARY IDS copied from the `b<n>` "
    "column at the left of the hunk body — never line numbers, never arithmetic. Both "
    "ends are inclusive; omit `end` for a single-line callout. Only lines carrying an id "
    "can bound a span; the `structure:` line under the hunk label gives each definition "
    "the hunk touches as a ready-made pair. Use a span for each semantically distinct "
    "edit when the hunk has several (a refactor plus an unrelated fix, a changed "
    "if-branch alongside a new else-branch), and a single-line span for a note too "
    "specific for `intent`. Spans may nest — a callout inside a region is fine — and "
    "need not cover the hunk; they must not partially overlap. A deleted line has no "
    "post-image line and no id: describe a deletion with a span on the line after it. "
    "Omit `spans` when the hunk is single-intent with nothing line-specific to say.\n"
    "- `smells`: list of {tag, note}. Tags are from the closed vocabulary: "
    f"{SMELL_TAGS_TEXT}. Attach each smell to a span when it's local to that range, or "
    "to the hunk when it spans the whole change.\n"
...
OVERVIEW_SYSTEM = (
    ...
    "  parse of what actually changed — ground truth, not your inference. Use it to "
    "  ground `summary`, `themes` and `groups`; do not restate it as a list.\n"
...
HUNK_SYSTEM = (
    ...
    "the user prompt with two left gutters: a boundary id (`b<n>`) on the lines a span "
    "may start or end on, and the POST-IMAGE (new-side) line number on every "
    "post-image line (deleted lines have blank gutters — they have no post-image line). "
    "A span's `start`/`end` are boundary ids copied from the first gutter; a same-file "
    "`refs[].line` is a number copied from the second. Never count lines yourself. "
...
HUNK_BATCH_SYSTEM = (
    ...
    "may start or end on, and the POST-IMAGE (new-side) line number on every "
    "post-image line (deleted lines have blank gutters — they have no post-image line). "
    "Boundary ids are numbered across the whole prompt, so an id belongs to exactly one "
    "hunk: a span's `start`/`end` are ids copied from that hunk's gutter; a same-file "
    "`refs[].line` is a number copied from the second gutter. Never count lines "
    "yourself. Each `intent` must name what THAT hunk "
...
EXPLAINER_SKELETON_GUIDANCE = (
    ...
    "Between them the three lists cover the Map: a file that earned a Map row names a "
    "part of the change, and that part belongs to whichever section will explain it. "
    "Background and Intuition stay as narrow as their bullets say, so Code is the home "
    "for what they do not take. Leaving a file out of all three is a decision that no "
    "prose will explain that part; make it deliberately, not by attrition.\n"
...
EXPLAINER_FIGURE_GUIDANCE = (
    ...
    "or count — a `<rect>` drawn behind a separate `<text>`, never a class on the text "
    "itself; `rule` a thick rounded separator.\n\n"
    "A class outside that list is removed, and so is one on the wrong kind of element: "
    "the text classes belong on `<text>` and `<tspan>`, every other class on a shape "
    "(`<rect>`, `<circle>`, `<ellipse>`, `<line>`, `<polyline>`, `<polygon>`, `<path>`). "
    "A container — `<g>`, `<svg>`, `<marker>`, `<defs>` — takes either and passes it to its "
    "children. Layout is yours — position, size and "
