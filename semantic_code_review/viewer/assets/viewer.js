// Semantic Code Review — viewer (orchestrator).
//
// The IIFE that loads DATA from the inline scr-data <script>, wires
// the modules together, and handles the few session-level pieces
// that don't naturally belong to any single module: the Done button,
// SSE → patch dispatch, and the per-event mutators that update DATA
// before delegating to window.ScrRender / window.ScrFolds.
//
...
(function () {
  ...
  function boot() {
    ...
    installDoneButton();
    window.ScrRender.init(DATA);       // wires hash + keyboard + initial paint
  ...
  // viewer.js's handlers patch the in-memory DATA + delegate the
  // visible side-effects to the right module.
  function installSessionEvents() {
    ...
    window.ScrSse.connect(SESSION_ENDPOINT, {
      ...
      hunkStart: (payload) => {
        ...
        window.ScrRender.repaintHunkHeader(hunkId);
      },
      hunk: (payload) => {
  ...
  function applyHunkPatch(payload) {
    ...
    window.ScrRender.renderHunkReplace(file, hi);
  }
  ...
  function applyFoldSummary(payload) {
    ...
    window.ScrRender.renderHunkReplace(f, hostHunkIdx);
    const fileEl = document.querySelector(
      '.file[data-id="' + cssEscape(f.id) + '"]',
    );
